...
list.of.packages <- c("data.table","utils","dplyr","lubridate","tidyr")
...
porcentaje <- as.double((colSums(is.na(flightsAux))/nrow(flightsAux))*100)
names <- colnames(flightsAux)
df <- data.frame(names)
df$porcentaje <- porcentaje
ggplot(df, aes(x = df$names, y = df$porcentaje)) + geom_col(fill="blue") + 
...
clenEmptyData <- function (dfFlightsAux){
  ...
  dfFlightsAux$TotalAddGTime <- NULL # 99.309710% de pérdida de información
  dfFlightsAux$LongestAddGTime <- NULL # 99.309710% de pérdida de información
  dfFlightsAux$DivReachedDest <- NULL # 99.776289% de pérdida de información
  dfFlightsAux$DivActualElapsedTime <- NULL # 99.833185% de pérdida de información
  dfFlightsAux$DivArrDelay <- NULL # 99.833185% de pérdida de información
  dfFlightsAux$DivDistance <- NULL # 99.778225% de pérdida de información
  dfFlightsAux$Div1AirportID <- NULL # 99.746487% de pérdida de información
  dfFlightsAux$Div1AirportSeqID <- NULL # 99.746487% de pérdida de información
  dfFlightsAux$Div1TotalGTime <- NULL # 99.746487% de pérdida de información
  dfFlightsAux$Div1LongestGTime <- NULL # 99.746487% de pérdida de información
  dfFlightsAux$Div2AirportID <- NULL # 99.996323% de pérdida de información
  dfFlightsAux$Div2AirportSeqID <- NULL # 99.996323% de pérdida de información
  dfFlightsAux$Div2TotalGTime <- NULL # 99.996323% de pérdida de información
  dfFlightsAux$Div2LongestGTime <- NULL # 99.996323% de pérdida de información
  dfFlightsAux$Div3AirportID <- NULL # 99.999806% de pérdida de información
  dfFlightsAux$Div3AirportSeqID <- NULL # 99.999806% de pérdida de información
  dfFlightsAux$Div3TotalGTime <- NULL # 99.999806% de pérdida de información
  dfFlightsAux$Div3LongestGTime <- NULL # 99.999806% de pérdida de información
...
# indica un 1, y un 0 cuando el vuelo no ha sido cancelado.
...
flights %>% 
  ...
  summarise(AirlineID = unique(AirlineID)) 
...
flights %>% 
  group_by(UniqueCarrier) %>% 
  summarise(Carrier = unique(Carrier)) 
...
# Compruebo si cada OriginAirportID tiene un único OriginAirporSeqID asociado.
flights %>% 
  group_by(OriginAirportID) %>% 
  summarise(OriginAirportSeqID = unique(OriginAirportSeqID))
# Cada OriginAirportID tiene un único OriginAirportSeqID asociado, por tanto se puede prescindir de
# la variable OriginAirportSeqID
...
# Compruebo si cada OriginAirportID tiene un único OriginCityMarketID asociado.
flights %>% 
  group_by(OriginAirportID) %>% 
  summarise(OriginCityMarketID = unique(OriginCityMarketID))
# Cada OriginAirportID tiene un único OriginCityMarketID asociado, por tanto se puede prescindir de
# la variable OriginCityMarketID
...
flights %>% 
  group_by(Origin) %>% 
  summarise(OriginAirportID = unique(OriginAirportID))
...
flights %>% 
  group_by(OriginCityName, Origin) %>% 
  summarise(OCN = unique(OriginCityName)) 
# En una misma ciudad puede haber más de un aeropuerto
# Compruebo si tiene NA's
flights %>% 
  filter(is.na(OriginCityName)) %>% 
  nrow()
# la variable no contiene ningún NA
...
flights %>% 
  ...
  summarise(OSF = unique(OriginStateFips)) 
...
# Compruebo si cada DestAirportID tiene un único DestAirportSeqID asociado.
flights %>% 
  group_by(DestAirportID) %>% 
  summarise(DestAirportSeqID = unique(DestAirportSeqID))
# Cada DestAirportID tiene un único DestAirportSeqID asociado, por tanto se puede prescindir de la 
# variable DestAirportSeqID
...
# Compruebo si cada Origin tiene un único OriginCityMarketID asociado.
flights %>% 
  group_by(DestAirportID) %>% 
  summarise(DestCityMarketID = unique(DestCityMarketID))
# Cada Origin tiene un único OriginCityMarketID asociado, por tanto se puede prescindir de la 
# variable OriginCityMarketID
...
flights %>% 
  group_by(Dest) %>% 
  summarise(DestAirportID = unique(DestAirportID))
...
flights %>% 
  group_by(DestCityName, Dest) %>% 
  summarise(OCN = unique(DestCityName)) 
# En una misma ciudad puede haber más de un aeropuerto
# Compruebo si tiene NA's
flights %>% 
  filter(is.na(DestCityName)) %>% 
  nrow()
# la variable no contiene ningún NA
...
flights %>% 
  group_by(DestState) %>% 
  summarise(OSF = unique(DestStateFips)) 
...
flights %>% 
  group_by(DestState) %>% 
  summarise(DestWac = unique(DestWac)) 
...
# Transformo la variable en formato hora
# flights$CRSDepTime = format(strptime(flights$CRSDepTime, format = "%H%M"), format = "%H%M")
...
# CRSDepTime <- flights$CRSDepTime
# as.difftime(CRSDepTime, format="%H",units="hours")
...
# Transformo la variable en formato hora
# flights$WheelsOff = format(strptime(flights$WheelsOff, format = "%H%M"), format = "%H%M")
summary(flights$WheelsOff)
# Compruebo si tiene NA's
flights %>% 
  filter(is.na(WheelsOff)) %>% 
  nrow()
# la variable no contiene ningún NA
...
# Transformo la variable en formato hora
# flights$WheelsOn = format(strptime(flights$WheelsOn, format = "%H%M"), format = "%H%M")
summary(flights$WheelsOn)
# Compruebo si tiene NA's
flights %>% 
  filter(is.na(WheelsOn)) %>% 
  nrow()
# la variable no contiene ningún NA
...
# hora local: hhmm.
# Transformo la variable en formato hora
# flights$CRSArrTime = format(strptime(flights$CRSArrTime, format = "%H%M"), format = "%H%M")
summary(flights$CRSArrTime)
# Compruebo si tiene NA's
flights %>% 
  filter(is.na(CRSArrTime)) %>% 
  nrow()
# la variable no contiene ningún NA
...
# Compruebo si tiene NA's
flights %>% 
  filter(is.na(ArrivalDelayGroups)) %>% 
  nrow()
# la variable no contiene ningún NA
...
# Compruebo si tiene NA's
flights %>% 
  filter(is.na(ArrTimeBlk)) %>% 
  nrow()
# la variable no contiene ningún NA


##### 1.5.7. Cancellations and Diversions #####

##### 1.5.7.1. CRSElapsedTime #####
...
##### 1.5.8. Cause of Delay #####

##### 1.5.8.1. CarrierDelay #####
...
flights %>% 
  filter(is.na(DistanceGroup)) %>% 
  nrow()
# la variable contiene 355878 NA's

##### 1.5.8.2. WeatherDelay #####
...
##### 1.5.9. Gate Return Information at Origin Airport  #####

##### 1.5.9.1. FirstDepTime  #####
str(flights$FirstDepTime)
# Esta variable contiene el retraso acumulado o reaccionario de la aeronave en minutos.
# Transformo la variable en formato hora
# flights$FirstDepTime = format(strptime(flights$FirstDepTime, format = "%H%M"), format = "%H%M")
summary(flights$FirstDepTime)
# Compruebo si tiene NA's
flights %>% 
  filter(FirstDepTime=="") %>% 
  nrow()
# la variable contiene 498794 NA's
flights %>% 
  filter(FirstDepTime=="") %>% 
  nrow()/nrow(flights)*100
# Esta variable contiene únicamente información en 3198 campos, tienen una pérdida de información
# del 99.36294% >99%, la elimino del dataset
